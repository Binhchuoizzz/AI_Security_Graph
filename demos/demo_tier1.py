...
print("   Lý do  : Port 22 ∈ sensitive_ports AND fwd_packets=5 < 200 → Block IP\n")
...
print("   Lý do  : fwd_packets=5000 > max_fwd_packets → Alert (không block: DDoS có thể spoof)\n")

# ─── Case 4: ESCALATE — Web attack (HTTP port, không volumetric) ──────────────
web_log = {"Source IP": "10.2.2.2", "Destination Port": 80, "Total Fwd Packets": 300, "payload": ""}
...
print(
    "   Lý do  : Port 80 ∈ [80,443,8080] nhưng fwd_packets=300 >= 200 (không brute-force) → ESCALATE lên Tier-2\n"
)
...
for port in [1, 2, 3, 25, 53, 135, 137, 139, 445, 3306, 5432, 8888]:
    result_scan = engine.evaluate(
        {"Source IP": "10.99.99.99", "Destination Port": port, "Total Fwd Packets": 1}
    )
print(f"   Action : {result_scan['tier1_action']}  | Score: {result_scan['tier1_score']}")
print(f"   Reasons: {result_scan['tier1_reasons']}")
print("   Lý do  : 12 non-HTTP ports > threshold(10) → AWAIT_HITL (Lateral Movement)\n")
...
print("   Lý do  : Source IP ∈ whitelist_ips → bỏ qua toàn bộ rules\n")
...
print(
    f"   Baseline Flow Duration: μ = {engine.global_stats['Flow Duration'].mean():.1f}, "
