...
{{
  ...
  "mitre_technique": "<ID - Name của kỹ thuật NGUY HIỂM NHẤT trong chuỗi tấn công (lấy từ RAG context). VD: nếu có cả Reconnaissance và Initial Access, hãy chọn Initial Access làm đại diện. Nếu không có kỹ thuật nào khớp 100% trong RAG context, tuyệt đối KHÔNG TỰ CHÉM (hallucinate), BẮT BUỘC chọn action là 'AWAIT_HITL' và ghi trường này là 'N/A'>",
  ...
  "reasoning": "<Phân tích CHI TIẾT bằng tiếng Việt (4-6 câu), văn phong mạch lạc của chuyên gia bảo mật, KHÔNG gạch đầu dòng máy móc. BẮT BUỘC TRÍCH DẪN BẰNG CHỨNG: mỗi khẳng định về hành vi PHẢI kèm ÍT NHẤT MỘT giá trị CỤ THỂ lấy nguyên văn từ log, viết dạng `tên_trường=giá_trị` (ví dụ: `Destination Port=22`, `Total Fwd Packets=900`, `message=' OR '1'='1`). TUYỆT ĐỐI CẤM các câu rỗng chỉ viện dẫn thẩm quyền như 'được xác nhận bởi MITRE ATT&CK và NIST' hay 'hành vi này nguy hiểm nên cần chặn' mà KHÔNG kèm số liệu — đó là lập luận vòng tròn, không phải bằng chứng. Cấu trúc: (1) tổng kết IP này thực hiện bao nhiêu hành vi đáng ngờ; (2) DẪN SỐ LIỆU THẬT từ log làm bằng chứng; (3) giải thích kẻ tấn công đang làm gì và vì sao các con số đó nguy hiểm; (4) nếu bạn SUY LUẬN thay vì đọc được trực tiếp từ log thì phải nói rõ 'log không có dấu hiệu trực tiếp, đây là suy luận theo hành vi'; (5) kết luận vì sao chọn hành động này. KHÔNG bịa trường hay giá trị không có trong log.>",
  "extracted_iocs": [
