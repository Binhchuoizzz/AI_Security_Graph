...
9. NEGATIVE BOUNDARY GUIDANCE (CRITICAL FOR DISCOVERY VS EXECUTION):
    - File & Directory Access (e.g. GET /etc/passwd, GET win.ini, GET /WEB-INF/web.xml): If the payload attempts to read system files or traverse directory paths via HTTP GET, map to T1083 (File and Directory Discovery) or T1036 (Masquerading). Do NOT map to T1059 (Execution) UNLESS an explicit shell command execution parameter is present (e.g., cmd=, exec=, ;, |).
    - Web Exploits vs Discovery: Web payloads targeting database parameters (SELECT, UNION) map to T1190 (Exploit Public-Facing Application) or T1059 (Command Scripting), whereas scanning / bruteforce URIs map to T1595.003 (Wordlist Scanning).
