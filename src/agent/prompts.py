...
4. ATTACK CHAIN ANALYSIS & CORRELATION (CRITICAL): When analyzing a batch of logs from the SAME Source IP, look for behavioral progression (e.g., from Reconnaissance/Port Scan to Initial Access/Brute-Force to Execution/SQLi). You MUST identify the full chain of events. If multiple attack phases exist across different logs in the batch, summarize the progression (e.g., "Reconnaissance -> Initial Access") and ESCALATE the severity. This indicates a multi-stage Kill-Chain or advanced persistent threat (APT).
...
{{
  ...
  "reasoning": "<Phân tích CHI TIẾT bằng tiếng Việt (4-6 câu) theo 4 phần: (1) BẰNG CHỨNG — đặc trưng CỤ THỂ quan sát được (cổng, số gói/byte, tần suất, Z-score...); (2) PHƯƠNG PHÁP & CHUỖI TẤN CÔNG — kẻ tấn công đang làm gì, có sự leo thang hành vi nào không (VD: 'IP này đang thực hiện Reconnaissance (Scan) sau đó leo thang sang Initial Access (Brute-Force). Mức độ: Nghiêm trọng'); (3) KỸ THUẬT — ID+tên MITRE nguy hiểm nhất và VÌ SAO khớp; (4) HÀNH ĐỘNG — vì sao chọn action này (luôn ưu tiên BLOCK_IP nếu phát hiện chuỗi leo thang).>",
  "extracted_iocs": [
