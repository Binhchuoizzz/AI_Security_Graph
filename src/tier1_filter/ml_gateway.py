...
class MLGateway:
    ...
    def _load_pipeline(self) -> dict[str, Any] | None:
        try:
            ...
            if not os.path.exists(model_path):
                logger.warning(f"[TIER-1.5 ML GATE] Không tìm thấy model tại {model_path}")
                return None
            with open(model_path, "rb") as f:
                return pickle.load(f)
        except Exception as e:
            logger.error(f"[TIER-1.5 ML GATE] Lỗi load mô hình: {e}")
            return None
    ...
    def evaluate(self, log: dict) -> tuple[str | None, str | None, float]:
        ...
        try:
            ...
            logger.error(f"[TIER-1.5 ML GATE] Lỗi dự đoán: {e}")
            return None, None, 0.0
        ...
        if confidence_attack >= self.conf_threshold_block:
            action = "BLOCK_IP"
            reasoning = f"Phát hiện tấn công bởi Cổng ML Tier-1.5 (LightGBM). Độ tin cậy: {confidence_attack:.2%}"
            confidence = float(confidence_attack)
        elif confidence_attack >= self.conf_threshold_alert:
            action = "ALERT"
            reasoning = f"Cảnh báo rủi cao bởi Cổng ML Tier-1.5 (LightGBM). Độ tin cậy: {confidence_attack:.2%}"
            confidence = float(confidence_attack)
        elif confidence_benign >= self.conf_threshold_block:
            action = "LOG"
            reasoning = f"Xác nhận an toàn bởi Cổng ML Tier-1.5 (LightGBM). Độ tin cậy: {confidence_benign:.2%}"
