...
Rather than employing a binary $F1$ metric—which misrepresents performance given an 86.9\% attack base rate—the system is evaluated via an \textit{action-based} method comparing Config A and Config F. Under Config F, human-in-the-loop escalation (\texttt{AWAIT\_HITL}) drops from 24.2\% to 0.0\%. Reasoning quality is independently graded by the Meta-Llama-3-8B-Instruct model on a 1-5 scale rather than self-evaluation. A score of 4.6/5.0 confirms robust forensic justification while eliminating the Self-Enhancement Bias commonly found in LLM-as-a-Judge protocols.
...
An ablation study on 300 balanced samples demonstrates that the Single-Tier LLM baseline suffers severe queue congestion. However, Configurations B, C, D, and E yield identical action and latency outcomes, proving that the binary action metric saturates on this dataset. Notably, latency measurements on the 50/50 dataset reveal that the new system is 7.8\% slower than anticipated, primarily caused by the continuous context switching between the LLM and the ML Gate when processing high-density fragmented event streams.
