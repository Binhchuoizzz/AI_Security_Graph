...
\section{Lỗ hổng Nhận thức LLM và các Vector Tấn công Đối kháng}
...
\section{Phát hiện Bất thường Thống kê Trực tuyến: Thuật toán Welford và Z-Score}
\label{sec:welford}
...
Nghiên cứu về phát hiện mối đe dọa tự động đã trải qua nhiều làn sóng. Phát hiện xâm nhập cổ điển dựa trên học máy (Random Forests, SVM) hoặc học sâu (LSTM, CNN)~\cite{mlids2016}; tuy chính xác, các mô hình này vận hành như những ``hộp đen'' thiếu tính giải thích. Sự xuất hiện tiếp theo của các Mô hình Ngôn ngữ Lớn đã mở ra một mặt trận nghiên cứu rộng khắp các tác vụ an ninh mạng, như được tổng hợp trong các khảo sát hệ thống gần đây trải trên hàng trăm công trình và hàng chục kịch bản ứng dụng~\cite{llmsec_survey2025}, thúc đẩy sự dịch chuyển từ các bộ phân loại mờ đục sang những hệ thống có thể giải thích và hành động trên phát hiện của mình.

Gần với luận văn này nhất là một làn sóng các khung agentic gần đây ghép suy luận LLM với truy xuất cho vận hành an ninh. CyberRAG~\cite{cyberrag2025} điều phối một tập bộ phân loại tinh chỉnh qua vòng lặp truy xuất và suy luận để gán nhãn và giải thích các tấn công web (SQLi, XSS, SSTI). LanG~\cite{lang2026} đề xuất một nền tảng nhận thức quản trị xây trên LangGraph, hợp nhất liên kết sự cố, sinh luật bằng LLM và bộ tái dựng tấn công ba giai đoạn với các chốt phê duyệt của con người. Khung tích hợp Splunk của Sahay và cộng sự~\cite{splunkllm2026} kết hợp một bộ tự mã hóa tái dựng và học tăng cường sâu cho phân loại sơ bộ với một LLM cho săn tìm mối đe dọa theo ngữ cảnh trong SIEM doanh nghiệp, và các thiết kế đa tác tử như AutoBnB-RAG~\cite{autobnb2025} mở rộng hợp tác tăng cường truy xuất sang điều tra ứng phó sự cố. Về phía đối kháng, Pandey và Bhujang~\cite{watchtower2026} không đề xuất hệ thống phòng thủ mà hình thức hóa mối đe dọa tiêm prompt từ nền log (phân loại S1--S4 ở trên) và chứng minh thực nghiệm tính hiệu quả của nó trước các hệ vận hành an ninh có LLM. Các hệ này khẳng định giá trị của tự động hóa SOC theo hướng agentic, neo bằng truy xuất, và phần lớn đã tích hợp một bộ tiền lọc, một số còn có cả rào chắn đầu vào.

Một hướng song song, đã trình bày ở phần trước của chương này, phơi bày cái giá an ninh khi đặt một mô hình sinh vào vòng quyết định: tấn công tiêm prompt gián tiếp từ nền log cho phép nội dung do kẻ tấn công kiểm soát chiếm quyền suy luận của mô hình~\cite{promptinjection2023, watchtower2026}, một rủi ro nay đã được hệ thống hóa trong hướng dẫn bảo mật ứng dụng cho các hệ LLM~\cite{owasp_llm2025}. Ở nơi các khung SOC hiện có có xử lý vấn đề này, chúng thường dựa vào một bộ phân loại ngữ nghĩa học được mà một tải trọng mới vẫn có thể qua mặt.

Trên bối cảnh đó, SENTINEL khác biệt ở ba trục thiết kế có chủ đích, chứ không phải ở một tính năng đơn lẻ mà họ thiếu. Thứ nhất, cổng Tầng 1 của nó là một thống kê Welford $O(1)$ không cần huấn luyện, thay vì một bộ tiền lọc học được (autoencoder và học tăng cường sâu~\cite{splunkllm2026}, hoặc các bộ phân loại tinh chỉnh~\cite{cyberrag2025}) vốn cần dữ liệu huấn luyện và tái huấn luyện định kỳ. Thứ hai, nó thay một bộ phân loại injection học được~\cite{lang2026} bằng cô lập có cấu trúc và tất định qua Đóng gói Dữ liệu Phân tách, một lá chắn đồng thuận hai tầng, và một chuỗi audit móc xích HMAC. Thứ ba, trái với các tác tử gọi công cụ có vòng lặp suy luận lặp đi lặp lại, nó chọn một điều phối phi chu trình, không gọi công cụ, chạy hoàn toàn trên một mô hình lượng tử hóa cục bộ duy nhất, ưu tiên tính tất định, khả năng kiểm toán và chủ quyền dữ liệu. Bảng~\ref{tab:related_work} định vị SENTINEL so với các hệ này trên những năng lực quan trọng nhất đối với một bộ máy SOC khả triển khai và được tăng cường bảo mật.
