...
> **Thực nghiệm bổ sung (ngoài 15 demo — rigor/độ bền/quan sát, xem mục #15 Cheat Sheet):** Ablation B–E (`run_ablation_bcde.py`) & cân bằng 150/150 (`run_ablation_balanced.py`), độ nhạy ngưỡng Welford (`run_threshold_sensitivity.py`), zero-day phân cấp (`run_zeroday_graded.py`), đối chứng âm APT + Wilson CI (`run_apt_negative_control.py`), stress ngữ cảnh (`run_context_stress.py`), độ bền LLM determinism + suy biến an toàn (`run_llm_robustness.py`), kháng adversarial full-pipeline (`evaluate_adversarial_pipeline.py`). Quan sát token ngữ cảnh runtime qua `src/agent/token_monitor.py` → `config/llm_token_stats.json`.
...
.venv/bin/python experiments/run_ablation_bcde.py
...
> **Lưu ý quan trọng (khớp code):** `run_ablation_study.py` thực tế chỉ chạy **hai đầu mút A và F** (sinh `Config_F.reasoning_outputs` cho `evaluate_reasoning.py`). Các cấu hình **B–E** được hiện thực trong `run_ablation_bcde.py` với trục cô lập đóng góp **gate Welford + từng tầng RAG** (xem cột "Runner thật" bên dưới) — KHÁC với thiết kế YAML khai báo ban đầu. Các tệp `config/ablation/*.yaml` là cấu hình KHAI BÁO (đối chiếu/tham khảo); runner thật là các script Python.

| Cấu Hình | Thiết kế YAML khai báo (`config/ablation/*.yaml`) | Runner thật & ngữ nghĩa đo |
| :--- | :--- | :--- |
| **Config A** | Rule Only (chỉ Rule Engine) | `run_ablation_study.py` — Tier-1 đầy đủ, KHÔNG LLM |
| **Config B** | LLM Only (không Tier-1) | `run_ablation_bcde.py` — Pure LLM (không gate/RAG/guardrails) |
| **Config C** | Không Encapsulation | `run_ablation_bcde.py` — Welford-gate + LLM (không RAG) |
| **Config D** | Chỉ RAG MITRE | `run_ablation_bcde.py` — gate + **dense-RAG** (FAISS-only) |
| **Config E** | Chỉ RAG NIST | `run_ablation_bcde.py` — gate + **hybrid-RAG** (FAISS+BM25+RRF) |
| **Config F** | **Full System (SENTINEL đầy đủ)** | `run_ablation_study.py` — full agent + Consensus Guard |

*Bản cân bằng 150/150 (`run_ablation_balanced.py`) chạy CẢ A–F trên cùng tập (150 benign + 150 tấn công đều 15 lớp) để phép so cấu phần có ý nghĩa — tránh hiệu ứng base-rate 93% tấn công làm mọi cấu hình suy biến về dự đoán toàn-dương.*
...
.venv/bin/python experiments/evaluate_adversarial_pipeline.py # Kháng adversarial FULL pipeline (Tier-2 LLM)
```
