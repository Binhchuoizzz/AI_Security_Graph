...
> **06/08/2026** · 20 chỉ số (đã loại 18, xem §4) · ✅ RQ1 8/8 · ✅ RQ2 6/6 · 🔄 RQ3 đang chạy
...
| **demo** | `data/demo.json` | 99.867 → **99.717** (bỏ 150 warmup) | `cicids_max` 91.311 · **DAPT2020** `dapt_max` 6.000 + `dapt` 402 · `cicids` 1.250 · zero-day 900 · đối kháng 4 · tấn công **9,77%**<br>🔴 **KHÔNG có CSIC** — luồng này không có tấn công tầng ứng dụng thật |
...
| 3.j | Chất lượng lập luận | `evaluate_reasoning.py` → 4 trục thang 1–5 + `evidence_grounding` | trọng tài LLM **KHÁC HỌ** chấm phán quyết của tác tử; `assert_cross_family()` chặn tự chấm mình | 277 lô leo thang LLM trong `ablation_results.json → Config_F` (ground_truth 1.750) | 🔄 xếp hàng — **phải sinh lại nguồn trước**: tệp ablation đang là bản 30/07, tức TRƯỚC bản vá "Tier-2 bị tước bằng chứng" |
| 3.l | Neo bằng chứng (0% ảo giác) | `score_evidence_grounding.py --trace ...` → `bad` · `la_chan_neo_kich_hoat` | hệ có KHẲNG ĐỊNH mã ATT&CK vắng trong tài liệu RAG của chính lô đó không | 500 bản ghi tracer của 3.b (bật `SENTINEL_TRACE=1`); chấm theo thước CHẶT = ID tài liệu đã truy xuất | ✅ **`bad` = 0/478 lô có khẳng định kỹ thuật** (0,00%)<br>lá chắn kích hoạt **22/500 = 4,4%**, cả 22 đều `BLOCK_IP → AWAIT_HITL` ⇒ **22 lệnh chặn IP vĩnh viễn tự động bị chặn lại**; cả 22 do **model** đề xuất, không phải bộ ánh xạ<br>kỹ thuật bị từ chối: T1190 ×6 · T1083 ×6 · T1595.003 ×4 · T1189 ×2 · T1110.003 ×2 · T1571 ×1 · T1110 ×1<br>⚠️ 16/22 mã bị từ chối **nằm trong từ vựng đáp án** ⇒ đây chính là chỗ 3.b mất điểm: lá chắn đổi **quy kết đúng** lấy **không tự chém** |
...
| 3.f | Ablation A↔F (tắt/đủ LLM) | A ≡ F, McNemar p = 1,00000 — không một ca nào khác nhau trong 1.750 mẫu |
| 3.g | Ablation B–E (đóng góp từng bậc RAG) | C ≡ D ≡ E DO CẤU TẠO: C/D/E chỉ gọi LLM khi Tier-1 leo thang (~17%), 83% còn lại rơi về cùng `tier1_verdict` |
| 3.h | Ablation khử base-rate | trên tập cân bằng A ≡ F (0,3133) và C ≡ D ≡ E (0,3167) — khác biệt ở tập lệch chỉ là hiện vật base-rate |
| 3.k | Đối chứng âm cho tương quan **có trạng thái** | 9 chuỗi DAPT, spec 1,0; DAPT đã xếp vào Hướng phát triển |
