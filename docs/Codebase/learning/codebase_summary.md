...
**Trạng thái:** `pytest 280+` · LangGraph 5 node (Cổng ML dời sang Tier-1) · golden baseline **BẬT mặc định**. **5D (benchmark cân bằng lại 2026-07-19: datatest 2514 = 15 lớp CICIDS đều tay + DAPT day2-5 + zero-day + adv; ground_truth 1250 = 16×80; chạy lại phần không-LLM):** Tier-1 gate F1 **0.531** (P.924/R.373), **Cổng ML (model 1M)** F1 **0.936** benchmark gộp (P.975/R.900) / 0.9635 test-190k · **giảm tải LLM 85.1%** F1(bypass) 0.991 (Config G, ground_truth 1250) · **kháng né-tránh ML 99.93%** (Inf/cực-đoan) · APT 3/3, zero-day 12/15 · độ trễ Tier-1 **~0.38ms** · adversarial Tier-2 **100%** · LLM-Judge **3.9/5** (cũ) · audit HMAC. ⚠️ Số cần-LLM (ablation B–F, reasoning, LLM-Judge) CHƯA rerun trên benchmark mới → còn phản ánh ground_truth cũ. Vận hành: [RUN_PROJECT.md](../guides/RUN_PROJECT.md); demo `scripts/run_demo.sh` (tự chạy `reset_all.py` trước).
