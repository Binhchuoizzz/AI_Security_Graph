...
| Mã | Nhiệm vụ cụ thể | Thời gian | Sản phẩm / Kết quả đầu ra | Trạng thái |
| :---: | :--- | :--- | :--- | :---: |
| **2.1** | **Xây dựng Lớp Guardrails Bảo mật**<br>- Triển khai Delimited Data Encapsulation chống Prompt Injection.<br>- Giải mã đa tầng Encoding Neutralizer (Base64, Hex, URL).<br>- Tích hợp bộ lọc Jailbreak, Drain3 log template miner và Output Sanitizer. | 16/06 – 25/06 | - Module `guardrails.py` với 5 lớp phòng thủ.<br>- Kịch bản kiểm thử bypass guardrails. | ⏳ |
...
| **3.2** | **Đánh giá Thực nghiệm Đa chiều (5D-EF)**<br>- Chạy Ablation Study (so sánh 6 cấu hình A-F) trên 750 mẫu GT.<br>- Đánh giá tính chịu lỗi trước 45 mẫu Adversarial.<br>- Đo đạc độ trễ (Latency baseline), kiểm định Mann-Whitney U.<br>- Chạy kiểm định LLM-as-Judge chéo họ mô hình. | 22/07 – 02/08 | - Báo cáo thực nghiệm tự động ghi nhận trên **MLflow**.<br>- Biểu đồ phân tích độ trễ và F1-Score cho luận văn. | ⏳ |
...
1. **Classification Quality (Chất lượng Phân loại):** Đo F1-Score, Precision, Recall trên tập Ground Truth 750 mẫu đại diện cho 15 loại tấn công. Mục tiêu đạt $F_1 \ge 0.90$.
...
3. **Adversarial Robustness (Độ bền vững):** Đánh giá khả năng chống chịu trước 45 mẫu tấn công nghịch đảo LLM (Prompt Injection, Base64 bypass, Jailbreak) vượt qua Guardrails.
