...
Hệ thống đề xuất sẽ được triển khai và đánh giá trên các bộ dữ liệu an ninh mạng chuẩn (ví dụ: CICIDS2017). Các tiêu chí đánh giá sẽ bao quát Chỉ số Phân loại (F1-Score), Độ trễ Vận hành, Mức độ Kháng lỗi trước các đòn tấn công adversarial, và Chất lượng Ngữ cảnh thông qua phương pháp kiểm định LLM-as-a-Judge.
...
Giai đoạn đầu tiên của quá trình triển khai sẽ tập trung vào Pipeline Tiếp nhận Dữ liệu và Động cơ Quy tắc Tier 1. Các bộ dữ liệu an ninh mạng tiêu chuẩn như CICIDS2017 hoặc UNSW-NB15 sẽ được tiền xử lý và mô phỏng thành luồng streaming thực tế. Cơ chế theo dõi phiên sẽ được xây dựng dựa trên chức năng TTL của Redis để thiết lập các đường cơ sở hành vi.
...
Nhằm kiểm chứng tính hiệu quả của hệ thống, nghiên cứu định nghĩa một chiến lược đánh giá toàn diện mang tên Khung Đánh giá 4D (4D Evaluation Framework).
...
\begin{itemize}
    \item \textbf{Chỉ số Phân loại:} Điểm F1-Score, Precision, Recall và Tỷ lệ Dương tính giả (FPR), phân tích bằng kiểm định McNemar để đảm bảo ý nghĩa thống kê.
    \item \textbf{Chỉ số Vận hành:} Độ trễ Suy luận (đại diện cho Thời gian Phát hiện Trung bình) và Tỷ lệ Cache Hit Ngữ nghĩa, được kiểm định thông qua phép thử Mann-Whitney U.
    \item \textbf{Chỉ số Kháng tấn công:} Tỷ lệ Đánh bại Guardrail sẽ được kiểm thử bằng các bộ dữ liệu adversarial chứa tấn công cấu trúc (Delimiter Smuggling) và sự nhầm lẫn ngữ nghĩa (Semantic Confusion).
    \item \textbf{Chỉ số Chất lượng Ngữ cảnh:} Dựa trên phương pháp tiếp cận LLM-as-a-Judge \cite{zheng2023}, quá trình suy luận của hệ thống sẽ được chấm điểm dựa trên Độ chính xác Ngữ cảnh, Sự liên quan của Câu trả lời và Tính trung thực (Faithfulness).
\end{itemize}
...
Đóng góp thứ hai nằm ở việc xây dựng phương pháp luận đánh giá các hệ thống SOC Agent dựa trên LLM. Thông qua Khung Đánh giá 4D — kết hợp giữa kiểm định thống kê, độ trễ vận hành và cơ chế chấm điểm LLM-as-a-Judge — nghiên cứu cung cấp một chuẩn mực (baseline) toàn diện để các nghiên cứu tương lai về an ninh mạng AI có thể tham chiếu.
