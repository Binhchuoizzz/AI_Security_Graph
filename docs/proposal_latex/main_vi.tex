...
Để giải quyết triệt để những thách thức này, nghiên cứu đề xuất một kiến trúc nhận thức (cognitive architecture) kết hợp giữa một Động cơ Quy tắc (Rule Engine) và một Mô hình Ngôn ngữ Lớn. Thông qua phương pháp tiếp cận hai tầng, hệ thống sàng lọc lượng lớn nhiễu ở tốc độ cao và phát hiện bất thường thống kê bằng thuật toán Welford online (Tầng 1), đồng thời bảo lưu năng lực suy luận sâu --- được vận hành bởi AI Tác tử dựa trên LangGraph cùng cơ chế Dual-RAG (tích hợp MITRE ATT\&CK cho nhận diện mối đe dọa và NIST SP 800-61r2 cho hướng dẫn ứng phó sự cố), kết hợp vòng lặp học tích cực Few-shot (Few-shot Active Learning) --- để xử lý các mối đe dọa phức tạp và khó đoán (Tầng 2). Bên cạnh đó, một cơ chế Đóng gói Dữ liệu có Phân cách (Delimited Data Encapsulation) cũng được giới thiệu nhằm vô hiệu hóa các nỗ lực chèn mã độc vào prompt mà không làm mất đi thông tin payload quan trọng cần thiết cho việc điều tra sự cố.
...
1. Việc áp dụng Kiến trúc Hai tầng (Rule-based + LLM Agent) kết hợp với Cơ sở Hành vi theo Phiên (Session-Aware Behavioral Baselining) và thuật toán phát hiện bất thường không giám sát Welford online có tối ưu hóa Độ trễ Suy luận so với phương pháp Một tầng (chỉ dùng LLM) hay không?
...
4. Một vòng lặp phản hồi (Feedback Loop) mang tính thích ứng với cơ chế Cách ly HITL (HITL Quarantine) và Few-shot Active Learning có thể ngăn chặn Adversarial Rule Injection và đảm bảo an toàn cho các phản ứng tự động ra sao?
...
Tại tầng đầu tiên (Tier 1), Động cơ Quy tắc đóng vai trò như một màng lọc heuristic tốc độ cao. Nó duy trì các đường cơ sở (baselines) hành vi nhận thức theo phiên bằng cách sử dụng cấu trúc dữ liệu trên bộ nhớ (như Redis) để theo dõi tần suất yêu cầu và các IP độc hại đã biết. Để chủ động phát hiện các cuộc tấn công zero-day hoặc bất thường chưa có signature, Tier 1 tích hợp một mô-đun phát hiện outlier không giám sát trực tuyến (Unsupervised Outlier Detector) sử dụng thuật toán Welford để tính toán giá trị Trung bình (Mean) và Độ lệch chuẩn (Std Dev) chạy với độ phức tạp $O(1)$ về thời gian và không gian trên các chỉ số luồng mạng chính (như Flow Duration, Fwd/Bwd Packets, Fwd/Bwd Length). Khi Z-score của một luồng vượt ngưỡng $|Z| > 3.5$, hệ thống sẽ tự động gán nhãn là bất thường phi luật lệ (Outlier Anomaly) và leo thang (escalate) trực tiếp lên Tier 2 để điều tra. Lớp này được thiết kế để sàng lọc phần lớn lưu lượng mạng lành tính hoặc nhiễu thể tích ngay tại tốc độ đường truyền (wire speed), giúp giảm trung bình 99.8\% thời gian xử lý log và đảm bảo LLM ở tầng dưới không bị quá tải bởi lưu lượng khổng lồ.
...
Để tối ưu hóa độ chính xác suy luận mà không cần fine-tune mô hình nền tảng, Tier 2 tích hợp vòng lặp học tích cực Few-shot (Few-shot Active Learning). Hệ thống tự động truy xuất các quyết định phê duyệt (Approve) hoặc bác bỏ (Reject) các luật trước đó của con người được lưu trong tệp cấu hình động để sinh ra các ví dụ few-shot đưa trực tiếp vào prompt ngữ cảnh của LLM. Cơ chế này giúp tác tử tự thích thích ứng động với các phán quyết của analyst, tránh lặp lại các phán quyết sai tương tự.
...
\textbf{Bảng điều khiển Web Dashboard hỗ trợ HITL (SOC UI):} Một giao diện quản trị chuyên nghiệp bằng Streamlit được xây dựng kèm phân quyền tài khoản (L1 Analyst và L3 Manager). Dashboard hiển thị thời gian thực hàng đợi cảnh báo, danh sách rule tường lửa đang chặn và mục quản trị cách ly. Streamlit Dashboard hiển thị thẻ chỉ số KPI đo lường Tỷ lệ cảnh báo sai thực tế (Live False Positive Rate - FPR) được tính toán động dựa trên các quyết định phê duyệt/từ chối thực tế nhằm phản ánh hiệu năng vận hành thực tế của hệ thống. Đồng thời, một mô-đun điều phối Docker Model Switcher được phát triển trên giao diện để cho phép người dùng chuyển đổi nhanh cấu hình hoặc hoán đổi mô hình LLM đang chạy (ví dụ giữa Gemma-2-9B-IT và Llama-3-8B-Instruct) một cách mượt mà thông qua việc cập nhật nóng tệp cấu hình môi trường. Dashboard cũng hỗ trợ tính năng Điều tra Đối tượng Tương tác (Interactive Threat Investigation) cho phép chuyên gia SOC click chọn trực tiếp địa chỉ IP bất kỳ trên bảng để truy xuất ngay điểm uy tín rủi ro, lập luận của AI, thời điểm First/Last Seen và dòng thời gian (timeline) sự kiện lịch sử từ SQLite.
