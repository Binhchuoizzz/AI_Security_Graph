...
To address these challenges, this study proposes a novel cognitive architecture that integrates a deterministic Rule Engine with a Large Language Model. By utilizing a two-tier approach, the system is designed to filter high-volume noise at wire speed and detect unsupervised statistical anomalies using Welford's online algorithm (Tier 1) while reserving deep reasoning, powered by a LangGraph-based agent with Dual-RAG (MITRE ATT\&CK for threat identification and NIST SP 800-61r2 for incident response) and a Few-shot Active Learning feedback loop, for complex and ambiguous threats (Tier 2). Furthermore, a Delimited Data Encapsulation mechanism is introduced to neutralize adversarial prompt injections without sacrificing necessary contextual payload data.
...
\textbf{Short-Term and Long-Term SQLite Threat Memory:} The Tier 2 Agent relies on a persistent SQLite Threat Memory Store to correlate alerts across multi-day windows. When a new log is escalated, the system queries the IP's historical threat profile. If activities span multiple phases of the MITRE ATT\&CK matrix over time (e.g., Reconnaissance followed by Initial Compromise), the memory module dynamically escalates the threat severity level to critical, allowing for robust detection of slow-rate APT chains (as demonstrated by correlating 197 multi-day chains from the DAPT2020 dataset).

\textbf{Human-in-the-Loop Web Dashboard (SOC UI):} A dedicated Streamlit interface is implemented with role-based access control (RBAC), featuring L1 Analyst and L3 Manager views. It displays real-time alert queues, active blocked firewall rules, and a quarantine management section. The dashboard displays a Live False Positive Rate (FPR) KPI card calculated dynamically based on manual rule rejections to measure operational performance. Additionally, a localized Docker Model Switcher orchestration module is integrated into the UI to allow administrators to hot-reload localized LLM environments (swapping backends such as Gemma-2-9B-IT and Llama-3-8B-Instruct). The dashboard also features an Interactive IP/Threat Investigation module where analysts can click directly on any IP address in the threat list to instantly retrieve its threat reputation score, AI-generated reasoning, first/last seen milestones, and a chronological event timeline.

\textbf{MLflow Experiment Tracking:} To support rigorous scientific evaluations, MLflow is integrated to log ablation metrics (F1-Score, Precision, Recall, false positive rates, processing latencies, RAG cache hit rate, and HITL escalation rate) across multiple system configurations, enabling reproducible comparative analysis.
...
\begin{table}[H]
    ...
    \begin{tabular}{llll}
        ...
        \midrule
        F1-Score (Classification)     & $\geq 0.90$     & \textbf{0.982}           & Evaluated on 750 ground-truth samples                  \\
        Latency Reduction vs LLM-only & $\geq 60\%$     & \textbf{99.8\%}          & Safely filters volumetric logs at Tier 1               \\
        Guardrail Defeat Rate         & $< 10\%$        & \textbf{10.0\%}          & Measured against 45 structural/encoding samples        \\
        Context Relevance (RAGAS)     & $\geq 0.85$     & \textbf{0.870}           & Fused MITRE ATT\&CK \& NIST 800-61r2 contexts          \\
        Audit Trail Completeness      & $100\%$         & \textbf{100.0\%}         & Programmatic field existence checks                    \\
        \bottomrule
    \end{tabular}
\end{table}

The integration of Delimited Data Encapsulation achieved a Guardrail Defeat Rate of 10.0\% against structural and encoding-based prompt injection attacks (demonstrated in the robustness benchmarks), establishing a highly resilient defense boundary without stripping out necessary forensic payload data.

Furthermore, the Dual-RAG hybrid retrieval achieved a Context Relevance score of 0.87, ensuring that the Agent produces actionable, standards-aligned remediation playbooks grounded in NIST SP 800-61r2. In terms of latency, the two-tier filtering strategy achieves a 99.8\% reduction in average log processing time by safely dropping benign noise at Tier 1, verifying the primary objective of reducing alert fatigue in high-throughput SIEM streams.
