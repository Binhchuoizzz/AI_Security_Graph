...
\textbf{Human-in-the-Loop Web Dashboard (SOC UI):} A dedicated Streamlit interface is implemented with role-based access control (RBAC), featuring L1 Analyst and L3 Manager views. It displays real-time alert queues, active blocked firewall rules, and a quarantine management section. The dashboard also features an Interactive IP/Threat Investigation module where analysts can click directly on any IP address in the threat list to instantly retrieve its threat reputation score, AI-generated reasoning, first/last seen milestones, and a chronological event timeline.
