...
Tài liệu này tổng hợp toàn bộ **41 tệp tin mã nguồn** trong hệ thống SENTINEL, được phân bổ theo **Luồng dữ liệu (Dataflow)** và xếp theo **Thứ tự lộ trình học tập 5 ngày** của học viên. Mỗi file đều được phân tích rõ: Mục đích, Tác dụng, và Mối quan hệ tương tác với các cấu phần khác.
...
*   **Mục đích:** Tự động tải và định dạng hóa các bộ dữ liệu an ninh mạng chuẩn phục vụ nghiên cứu.
*   **Tác dụng:** Tải tệp dữ liệu tĩnh CSV của bộ dữ liệu CSE-CIC-IDS2018 và DAPT2020 từ nguồn công khai, thực hiện ETL ban đầu để làm sạch dữ liệu.
*   **Mối quan hệ:** Đầu ra ghi vào thư mục `data/`. Được gọi thủ công khi cài đặt môi trường.

### 2. `scripts/build_dataset.py`
*   **Mục đích:** Xử lý chuyên sâu dữ liệu thô và phân chia các tập train/test.
*   **Tác dụng:** Đọc dữ liệu từ `data/`, chuyển đổi định dạng và dán nhãn các chuỗi tấn công Kill-chain của DAPT2020 trước khi nạp vào stream.
*   **Mối quan hệ:** Phụ thuộc vào dữ liệu do `fetch_and_build_dataset.py` tải về.

### 3. `src/streaming/publisher.py`
...
### 6. `src/tier1_filter/rule_engine.py` *(Cực kỳ quan trọng)*
...
### 7. `src/tier1_filter/feedback_listener.py`
...
## **NGÀY 2: TẦNG AN TOÀN VÀ N N DỮ LIỆU (GUARDRAILS)**

### 10. `src/guardrails/template_miner.py`
...
### 11. `src/guardrails/prompt_filter.py`
...
### 12. `src/guardrails/output_sanitizer.py`
...
### 13. `src/guardrails/data_validator.py`
...
### 14. `src/guardrails/state_monitor.py`
...
### 17. `scripts/build_rag_indexes.py`
...
### 18. `src/rag/retriever.py` *(Cực kỳ quan trọng)*
...
### 25. `src/agent/nodes.py` *(Cực kỳ quan trọng)*
...
### 34. `experiments/run_ablation_study.py`
...
### 35. `experiments/statistical_tests.py` *(Cực kỳ quan trọng)*
...
### 36. `experiments/evaluate_robustness.py`
...
*   **Mối quan hệ:** Đánh giá độc lập tính an toàn prompt.

### 37. `experiments/evaluate_reasoning.py`
...
### 38. `experiments/evaluate_zeroday.py`
...
### 39. `experiments/measure_latency_baseline.py`
...
### 40. `experiments/plot_results.py`
