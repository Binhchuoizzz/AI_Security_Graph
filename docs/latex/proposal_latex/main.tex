...
\begin{titlepage}
    ...
    \begin{tabular}{rl}
        ...
        \textbf{Supervisors:} & 1. Dr. Bùi Văn Hiệu   \\
                              & 2. Dr. Đặng Văn Hiếu  \\
    \end{tabular}
...
Based on the objectives, this thesis addresses three core research questions. The first (RQ1) asks whether a Two-Tier architecture---a rule-based engine coupled with an LLM agent and augmented by session-aware behavioural baselining and unsupervised online Welford anomaly detection---optimises reasoning latency relative to a one-tier, LLM-only approach. The second (RQ2) asks whether a Delimited Data Encapsulation mechanism using dynamic randomised delimiters, reinforced by a deterministic Tier-1/Tier-2 consensus guard and a Human-in-the-Loop quarantine, can prevent structural prompt injections such as delimiter smuggling---together with unsafe autonomous responses---without degrading the LLM's classification accuracy. The third (RQ3) asks how a Dual-RAG pipeline that combines a tactical threat framework with incident-response standards improves the contextual relevance of the LLM's threat analysis.
...
Early intrusion detection relied predominantly on signature matching---as exemplified by Snort and Suricata---to recognise previously catalogued threats. Although signature matching sustains very high throughput, it is structurally unable to detect zero-day exploits or polymorphic malware, whose mutating payloads evade any fixed signature \cite{khraisat2019}.

The subsequent adoption of machine learning gave rise to anomaly-based detection, in which models such as Random Forests, LSTMs, and log-specific transformers like LogBERT \cite{logbert2021} learn a profile of normal behaviour and flag departures from it. Although this markedly improved zero-day sensitivity, such models remain confined to pattern-based anomaly scoring: because they consult no external knowledge at inference time, they can neither articulate \textit{why} a log sequence is malicious, nor map an observation onto a standardised threat taxonomy, nor propose actionable remediation---capabilities that intrinsically require the contextual reasoning of a Large Language Model.
...
More recently, Large Language Models (LLMs) have opened new avenues for automated log summarisation and incident response. A comprehensive survey by Habibzadeh et al.\ \cite{habibzadeh2025} documents the rapid uptake of cloud-hosted LLM APIs such as GPT-4 for SOC automation, spanning triage and summarisation. Routing sensitive telemetry to third-party endpoints, however, conflicts with strict data-sovereignty policies and incurs network latency that is unacceptable for real-time triage.

For knowledge-grounded analysis, graph-based threat-intelligence methods \cite{attackg2023} have shown that attack techniques can be extracted from unstructured CTI reports and organised into MITRE ATT\&CK-aligned graphs. Such efforts nonetheless concentrate on tactical identification alone, omitting the operational incident-response standards---notably NIST SP 800-61r2---that are required to translate an identification into a concrete remediation.

On the security side, both the OWASP Top 10 for LLM Applications \cite{owasp2025} and the Agent Security Bench \cite{zhang2024asb} report that LLM agents remain highly susceptible to prompt-injection attacks. The Drain log-parsing algorithm \cite{he2017}, meanwhile, is widely adopted for template mining but is conventionally applied to storage compression rather than as an active injection defence. Finally, the LLM-evaluation literature increasingly cautions against self-enhancement bias, recommending cross-family judging or statistical validation in preference to a single self-evaluating judge \cite{zheng2023, ragas2023}. Taken together, these strands expose the gap that this work targets: the absence of a unified, self-defending architecture subjected to rigorous, multi-dimensional evaluation.
...
\begin{figure}[!htbp]
    ...
    \node[box, text width=7cm, below=1cm of tier1] (guardrail) {GUARDRAIL LAYER \\ 1. Drain template mining -- token compression \\ 2. Delimited Data Encapsulation -- anti-inject};
...
Logs that exhibit anomalous behaviour or trigger ambiguous thresholds are passed to the Guardrail Layer, where a two-step neutralisation occurs. First, template mining with the Drain algorithm extracts the static log template and compresses the token volume without altering the dynamic payload variables. Second, Delimited Data Encapsulation wraps the dynamic payload fields within cryptographically generated randomised delimiters (for example, \verb|<<RANDOM_HEX>>|), preventing an adversary from using delimiter smuggling to escape the data context and execute a prompt injection.

Following the guardrails, the sanitised log enters Tier 2, governed by a LangGraph-based agent that employs a Dual-RAG mechanism over two dedicated FAISS indexes, queried sequentially and merged before LLM inference. The first index, built on MITRE ATT\&CK, maps the observed indicators to specific tactics, techniques, and procedures (TTPs) and thus answers \textit{``what attack is happening?''} The second index, built on NIST SP 800-61r2 \cite{nist80061}, retrieves the appropriate incident-response phase---preparation, detection, containment, eradication, recovery, or lessons learned---and thus answers \textit{``what should the SOC do next?''}
...
The implementation process will first focus on data ingestion and the Tier-1 Rule Engine, evaluated on two benchmark datasets. CSE-CIC-IDS2018 \cite{sharafaldin2018} serves as the de facto standard for enterprise network intrusion-detection research; as of 2025 no publicly available general-purpose enterprise NIDS dataset post-2018 exists, since the CIC 2023--2026 releases (CICIoT2023, CIC-YNU-IoTMal 2026) target IoT environments that are not representative of enterprise SOC traffic. Because CSE-CIC-IDS2018 provides both raw PCAP and pre-extracted flow features and is widely adopted, it also enables direct comparison with prior work. DAPT2020 \cite{dapt2020}, a purpose-built dataset for multi-stage Advanced Persistent Threat detection, complements it by contributing the attack chains that exercise SENTINEL's Tier-2 agent and long-term Threat Memory.

The session-tracking mechanism will be built using Redis TTLs to establish behavioral baselines.

Following this, the Guardrail Layer will be implemented, integrating the Drain algorithm for template mining and developing the cryptographic Delimited Data Encapsulation module to neutralize adversarial payloads.
...
To evaluate the system's baseline accuracy, an Ablation Study will be conducted. This contrasts the full two-tier system (Configuration~F) against a deterministic Tier-1-only baseline (Configuration~A) to isolate the latency and verdict contribution of the cognitive tier; an LLM-only configuration is retained as a planned extension for finer-grained, per-component isolation.
The evaluation spans five complementary dimensions. \textbf{Classification} is captured by F1-score, precision, recall, and false-positive rate, with statistical significance assessed by McNemar's test ($p<0.05$) \cite{mcnemar1947}. \textbf{Operational} performance is measured through processing latency (a proxy for mean time to detect and respond), the HITL escalation rate, and the RAG semantic-cache hit rate, with significance assessed by the Mann-Whitney~U test ($p<0.05$) \cite{mannwhitney1947}. \textbf{Robustness} is quantified as the static guardrail block rate against 120 curated adversarial payloads spanning five OWASP-aligned categories \cite{owasp2025} (encoding bypass, structural/delimiter attacks, semantic confusion, jailbreak, and RAG poisoning). \textbf{Context quality} is evaluated by a cross-family LLM-as-a-Judge protocol that mitigates self-enhancement bias: while Gemma-2-9B-IT is the primary reasoning model, a different-family judge---Llama-3-8B-Instruct---scores Context Precision, Answer Relevancy, Faithfulness, and Context Recall under the RAGAS framework \cite{ragas2023}, cross-validated against automated RAGAS metrics. Finally, \textbf{explainability} is measured deterministically as the audit-trail completeness rate through programmatic field-presence checks.

\subsection{Project Timeline}
The project spans a standard 12-week Capstone period, structured as follows:
...
\begin{figure}[H]
    ...
    \gantttitle{12-Week Capstone Project Timeline}{12} \\
    \gantttitlelist{1,...,12}{1} \\
    \ganttbar{Literature Review \& Dataset}{1}{2} \\
    \ganttbar{Tier 1: Rule Engine \& Redis}{3}{4} \\
    \ganttbar{Guardrails (Drain \& Encapsulation)}{5}{6} \\
    \ganttbar{Local LLM backend \& LangGraph}{7}{8} \\
    \ganttbar{Dual-RAG (MITRE \& NIST)}{9}{9} \\
    \ganttbar{HITL Dashboard \& Audit DB}{10}{10} \\
    \ganttbar{Ablation Study \& Evaluation}{11}{11} \\
    \ganttbar{Final Report \& Defense}{12}{12}
    \end{ganttchart}
    }
