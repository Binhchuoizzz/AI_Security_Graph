...
Dựa trên các mục tiêu đã đề ra, luận văn giải quyết ba câu hỏi nghiên cứu cốt lõi. Câu hỏi thứ nhất (RQ1) đặt vấn đề liệu một Kiến trúc Hai tầng --- bộ máy dựa trên luật kết hợp với một tác tử LLM, được tăng cường bởi cơ sở hành vi theo phiên và phát hiện bất thường trực tuyến phi giám sát Welford --- có tối ưu hóa độ trễ suy luận so với phương pháp một tầng chỉ-dùng-LLM hay không. Câu hỏi thứ hai (RQ2) đặt vấn đề liệu cơ chế Đóng gói Dữ liệu có Phân cách (dùng dấu phân cách ngẫu nhiên động), được tăng cường bởi lá chắn đồng thuận Tầng 1/Tầng 2 tất định và cơ chế Cách ly Human-in-the-Loop, có thể ngăn chặn các đòn tiêm prompt mang tính cấu trúc như delimiter smuggling --- cùng các phản ứng tự động không an toàn --- mà không làm suy giảm độ chính xác phân loại của LLM hay không. Câu hỏi thứ ba (RQ3) đặt vấn đề một đường ống Dual-RAG kết hợp khung chiến thuật với các tiêu chuẩn ứng phó sự cố cải thiện độ phù hợp ngữ cảnh của phân tích rủi ro do LLM thực hiện như thế nào.
...
\begin{figure}[!htbp]
    ...
    \node[box, text width=7.5cm, below=1cm of tier1] (guardrail) {TẦNG RÀO CHẮN (GUARDRAIL) \\ 1. Gom nhóm template Drain -- nén token \\ 2. Đóng gói phân cách -- chống chèn mã độc};
...
Đối với những log có biểu hiện bất thường hoặc vượt qua ngưỡng mập mờ, chúng được chuyển đến Tầng Guardrail, nơi quá trình vô hiệu hóa diễn ra theo hai bước. Trước hết, việc gom nhóm khuôn mẫu bằng thuật toán Drain trích xuất cấu trúc tĩnh của log và nén số lượng token mà không làm thay đổi các biến payload động. Sau đó, cơ chế Đóng gói Dữ liệu có Phân cách bao bọc các trường payload động bên trong những dấu phân cách sinh ngẫu nhiên theo chuẩn mật mã (ví dụ: \verb|<<RANDOM_HEX>>|), ngăn kẻ tấn công dùng delimiter smuggling để thoát khỏi ngữ cảnh dữ liệu và thực thi một đòn tiêm prompt.

Sau khi vượt qua guardrail, dữ liệu log sạch tiến vào Tier 2, được quản lý bởi một Agent trên nền LangGraph vận dụng cơ chế Dual-RAG trên hai chỉ mục FAISS riêng biệt, được truy vấn tuần tự và hợp nhất ngữ cảnh trước khi suy luận LLM. Chỉ mục thứ nhất, xây trên MITRE ATT\&CK, ánh xạ các chỉ báo quan sát được vào các chiến thuật, kỹ thuật và thủ tục (TTP) cụ thể, qua đó trả lời câu hỏi \textit{``cuộc tấn công nào đang xảy ra?''} Chỉ mục thứ hai, xây trên NIST SP 800-61r2 \cite{nist80061}, truy xuất giai đoạn ứng phó sự cố phù hợp --- Chuẩn bị, Phát hiện, Ngăn chặn, Loại bỏ, Khôi phục, hay Bài học kinh nghiệm --- qua đó trả lời câu hỏi \textit{``SOC cần làm gì tiếp theo?''}
...
Quá trình triển khai trước tiên tập trung vào việc thu nhận dữ liệu và Động cơ Quy tắc Tier 1, được đánh giá trên hai bộ dữ liệu chuẩn. CSE-CIC-IDS2018 \cite{sharafaldin2018} đóng vai trò tiêu chuẩn thực tế cho nghiên cứu phát hiện xâm nhập mạng doanh nghiệp; vì cung cấp cả tệp PCAP thô lẫn các đặc trưng luồng trích xuất sẵn và được sử dụng rộng rãi, nó cho phép so sánh trực tiếp với các nghiên cứu trước. DAPT2020 \cite{dapt2020}, một bộ dữ liệu thiết kế riêng cho phát hiện tấn công APT đa giai đoạn, bổ sung cho CSE-CIC-IDS2018 bằng các chuỗi tấn công dài hạn nhằm thử nghiệm tác tử Tier 2 và Bộ nhớ Đe dọa dài hạn của SENTINEL.
...
Tiếp theo, lớp Guardrail được triển khai, tích hợp thuật toán Drain cho gom nhóm log và phát triển mô-đun Đóng gói Dữ liệu có Phân cách bằng mật mã để trung hòa các payload độc hại.
...
Quá trình đánh giá trải trên năm chiều bổ trợ. Về \textbf{Phân loại}, hệ thống được đo bằng F1-Score, Precision, Recall và Tỷ lệ Dương tính giả (FPR), với ý nghĩa thống kê được kiểm chứng bằng kiểm định McNemar ($p<0.05$) \cite{mcnemar1947}. Về \textbf{Vận hành}, các chỉ số gồm độ trễ xử lý (đại diện cho thời gian phát hiện/phản hồi trung bình), tỷ lệ leo thang HITL và tỷ lệ trúng cache ngữ nghĩa RAG, với ý nghĩa thống kê kiểm chứng bằng phép thử Mann-Whitney~U ($p<0.05$) \cite{mannwhitney1947}. Về \textbf{Kháng lỗi}, tỷ lệ chặn của rào chắn tĩnh được đo trên 120 tải trọng đối kháng thuộc năm nhóm theo chuẩn OWASP \cite{owasp2025} (ngụy trang mã hóa, tấn công cấu trúc/dấu phân tách, gây nhiễu ngữ nghĩa, bẻ khóa, và đầu độc RAG). Về \textbf{Chất lượng Ngữ cảnh}, một quy trình LLM-as-a-Judge chéo họ giúp khử thiên kiến tự đề cao: trong khi Gemma-2-9B-IT là mô hình suy luận chính, một giám khảo khác họ là Llama-3-8B-Instruct chấm Độ chính xác Ngữ cảnh, Sự liên quan của Câu trả lời, Tính trung thực và Độ bao phủ Ngữ cảnh theo khung RAGAS \cite{ragas2023}, được kiểm chứng chéo với các chỉ số RAGAS tự động. Cuối cùng, về \textbf{Tính Giải thích}, tỷ lệ hoàn thiện nhật ký kiểm toán được đo một cách xác định qua kiểm tra tự động sự hiện diện của các trường dữ liệu.

\subsection{Kế hoạch Thực hiện}
Dự án được triển khai trong khuôn khổ 12 tuần tiêu chuẩn của Đồ án Tốt nghiệp (Capstone), cấu trúc như sau:

\begin{table}[H]
    \centering
    \caption{Kế hoạch Thực hiện 12 Tuần}
    \label{tab:timeline}
    \begin{tabular}{lp{10cm}}
        ...
        Tuần 5--6 & Cài đặt Lớp Guardrail (Drain và Đóng gói có Phân cách). \\
        Tuần 7--8 & Tích hợp backend LLM cục bộ (llama.cpp) và luồng LangGraph. \\
        Tuần 9 & Xây dựng chỉ mục Dual-RAG FAISS (MITRE ATT\&CK và NIST SP 800-61r2). \\
        Tuần 10 & Phát triển Bảng điều khiển Streamlit HITL và CSDL Kiểm toán SQLite. \\
        Tuần 11 & Tích hợp toàn hệ thống, chạy Thí nghiệm Loại bỏ và thu thập số liệu. \\
        Tuần 12 & Viết báo cáo tổng kết, chuẩn bị bảo vệ và nộp dự án. \\
        \bottomrule
    \end{tabular}
\end{table}
