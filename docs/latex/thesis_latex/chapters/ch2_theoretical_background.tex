...
Furthermore, running multi-billion parameter models locally presents substantial memory requirements. For instance, executing a 9-billion parameter model like \textit{Gemma-2-9B-IT} in 16-bit floating-point (FP16) requires roughly 18GB of VRAM. To enable local inference on consumer-grade hardware, model quantization is applied. Quantization mathematically projects continuous weight parameters (such as FP16) onto discrete, lower-bit representations (such as INT4) \cite{quantization2021}. Utilizing GGUF formats and the Q6\_K quantization standard via the \texttt{llama.cpp} framework reduces the memory footprint to approximately 7--8\,GB of VRAM---a reduction of over 50\%---maximizing inference speed with minimal degradation in model reasoning.

\section{Agentic AI Architecture and State-Graph Models}

To address these challenges, automated security systems are shifting from rigid Runbook Automation (RBA) to autonomous Agentic AI systems. The core operational differences between these two paradigms include:
\begin{itemize}
    \item \textbf{Runbook Automation (RBA):} Relies on static, pre-defined rules and deterministic scripts for linear alert handling, which is highly efficient but structurally blind to zero-day conditions or polymorphic alert formats.
    \item \textbf{Agentic AI Systems:} Exhibit cognitive autonomy with capacities for self-reflection, multi-step planning, and adaptive context routing, allowing them to dynamically adjust analysis based on intermediate security findings.
\end{itemize}
...
\begin{figure}[!htbp]
    ...
    \caption{State-Graph (FSM) based workflow of an Agentic AI system during security incident analysis. (Source: Author)}
    \label{fig:fsm_workflow}
\end{figure}

Stateful multi-actor orchestration paradigms \cite{langgraph2024} allow agents to retain contextual working memory across multiple processing loops, enabling iterative decision refinement. This ensures that the agent can review previous triage decisions and collect additional evidence before issuing a final response.
...
Recent work formalizes these log-substrate attacks into a four-class taxonomy \cite{watchtower2026}:
\begin{itemize}
    \item \textbf{Direct Override (S1):} The adversarial payload openly instructs the model to ignore prior security directives and override system instructions.
    \item \textbf{Persona Hijack (S2):} The payload coerces the model into adopting an unrestricted or unauthorized persona to bypass safety alignment rules.
    \item \textbf{Context Manipulation (S3):} The payload fabricates authorization metadata or false provenance (e.g., claiming an alert was pre-approved or whitelisted) within the log stream.
    \item \textbf{Obfuscated Payloads (S4):} The instructions are concealed using Base64, hexadecimal, URL encoding, or homoglyph substitutions to evade simple regex pattern filters.
\end{itemize}
Because the log fields carrying these payloads are attacker-controlled yet structurally indistinguishable from legitimate evidence, a single-pass LLM analyst cannot reliably separate data from instructions. This taxonomy directly motivates the layered, defense-in-depth guardrail design adopted in Chapter~\ref{ch:system_design_implementation}.
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{|>{\raggedright\arraybackslash}p{0.30\textwidth}|c|c|c|c|c|}
        \hline
        \textbf{Capability} & \textbf{SENTINEL} & \textbf{CyberRAG} & \textbf{LanG} & \textbf{Splunk-LLM} & \textbf{Watchtower} \\
         & (ours) & \cite{cyberrag2025} & \cite{lang2026} & \cite{splunkllm2026} & \cite{watchtower2026} \\
        \hline
        Primary focus & 2-tier defense & Attack classification & Unified governance & Threat hunting & Attack study \\
        \hline
