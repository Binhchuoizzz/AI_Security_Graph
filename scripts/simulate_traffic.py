...
def stream_logs_to_redis():
    ...
    print(
        f"[*] Found {total_samples} samples. Throttling ingestion with {BATCH_DELAY_SECONDS}s delay..."
    )
    ...
    try:
        for index, sample in enumerate(samples):
            ...
            print(
                f"[>] Published sample {index+1}/{total_samples} (ID: {sample['id']}) -> Queue: {target_queue}"
            )
