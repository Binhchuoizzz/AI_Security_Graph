...
def main():
    ...
    listener.receive_new_rule("Source IP", IP_A, score=100, source="manual_ui", reason="Block từ UI")
    ...
    assert res_a2['tier1_action'] == 'BLOCK_IP', f"Lỗi: IP A không bị chặn ở Tier-1. Hành động: {res_a2['tier1_action']}"
    ...
    print_result("Lần 2 (Đã Whitelist có payload SQLi)", IP_B, res_b2['tier1_action'], res_b2['tier1_reasons'])
    assert res_b2['tier1_action'] == 'WHITELIST_DROP', f"Lỗi: IP B không được Whitelist_drop. Hành động: {res_b2['tier1_action']}"
    assert any("SQLi" in str(r) for r in res_b2['tier1_reasons']) or any("WAF" in str(r) for r in res_b2['tier1_reasons']), "Lỗi: Whitelist không phân tích tiếp log (không thấy SQLi reason)."
    ...
    assert res_c2['tier1_action'] != 'BLOCK_IP', f"Lỗi: IP C vẫn bị block. Hành động: {res_c2['tier1_action']}"
    ...
    print_result("Lần 3 (Đã Bỏ Whitelist, có payload bẩn)", IP_B, res_b3['tier1_action'], res_b3['tier1_reasons'])
    assert res_b3['tier1_action'] != 'WHITELIST_DROP', f"Lỗi: IP B vẫn pass whitelist. Hành động: {res_b3['tier1_action']}"
