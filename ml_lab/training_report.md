...
- **Lưu ý Kiến trúc Hệ thống:** Vì model xuất sắc nhất là **LightGBM** (hoặc Decision Tree tuỳ lần chạy được pickle), nên file mô hình được xuất ra dưới định dạng Tự Điển (`dict`) bao gồm `scaler`, `model`, và danh sách `features`. File này được lưu ở `tier_2_model.pkl` (tên file giữ theo lịch sử) và được Cổng ML của Tier-1 nạp lúc chạy. Mọi log mới nếu bị model từ chối do độ tin cậy thấp sẽ tự động escalate lên **Tier-2 (LLM Agent)** đánh giá ngữ cảnh.
