...
- **Tier 1.5 — Cổng ML (Mục tiêu của Lab này)**: Chặng đầu của Tier-2, dùng mô hình Machine Learning siêu nhẹ (như LightGBM, Decision Tree) làm bộ lọc Early Exit. Nhiệm vụ là chặn đứng ngay lập tức các cuộc tấn công DDoS/Brute-force rõ ràng (đã biết), tiết kiệm chi phí gọi API LLM.
...
- **Lưu ý Kiến trúc Hệ thống:** Vì model xuất sắc nhất là **LightGBM** (hoặc Decision Tree tuỳ lần chạy được pickle), nên file mô hình được xuất ra dưới định dạng Tự Điển (`dict`) bao gồm `scaler`, `model`, và danh sách `features`. File này được lưu ở `tier_2_model.pkl` và được Node ML Triage đọc vào lúc chạy Agent_UI. Mọi log mới nếu bị model từ chối do độ tin cậy thấp sẽ tự động chuyển tiếp sang LLM (chặng sau của Tier-2) đánh giá ngữ cảnh.
